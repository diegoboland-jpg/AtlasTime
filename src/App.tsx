...
import { bestHour, dateAtUtcHour, formatInZone, hourInZone, localLabel, scoreHours, timeZones } from "./time";
...
export default function App() {
  ...
  const hours = useMemo(() => scoreHours(people, dateValue), [people, dateValue]);
  const recommendation = useMemo(() => bestHour(people, dateValue), [people, dateValue]);
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <span className="mvp-badge">MVP 0.1</span>
      </header>
      ...
      <main>
        <section className="hero">
          <div>
            ...
            <p className="hero-copy">
              Compare local hours, spot shared availability, and move directly to the call.
            </p>
          </div>
        ...
        <section className="section">
          ...
          {showForm && (
            <form className="add-form" onSubmit={addPerson}>
              <label>
                Name
                <input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Olesya" autoFocus />
              </label>
              <label>
                City
                <input value={city} onChange={(event) => setCity(event.target.value)} placeholder="e.g. Moscow" />
              </label>
              ...
              <button className="primary-button form-submit" type="submit">Save person</button>
            </form>
          )}
          ...
          <div className="people-grid">
            {people.map((person) => {
              ...
              const status = localHour >= person.workStart && localHour < person.workEnd ? "Working hours" : "Outside work hours";
              return (
                <article className="person-card" key={person.id}>
                  <button className="icon-button remove" onClick={() => removePerson(person.id)} aria-label={`Remove ${person.name}`}>
                    <Trash2 size={16} />
                  </button>
                  ...
                  <div className="person-main">
                    ...
                    <span className={status === "Working hours" ? "status online" : "status"}>
                      {status}
                    </span>
                  </div>
                  <div className="person-time">
                    <strong>{formatInZone(now, person.timeZone)}</strong>
                    <span>{formatInZone(now, person.timeZone, { weekday: "short", day: "2-digit", month: "short" })}</span>
                  </div>
                </article>
              );
            })}
        ...
        <section className="section planner">
          ...
          {recommendation && (
            <div className="recommendation">
              <div className="recommendation-icon"><Clock3 size={24} /></div>
              <div>
          ...
          <div className="timeline-wrap">
            <div className="timeline-labels">
              <span>UTC</span>
              {hours.map((hour) => <span key={hour.utcHour}>{String(hour.utcHour).padStart(2, "0")}</span>)}
            </div>
            {people.map((person) => (
              <div className="timeline-row" key={person.id}>
                ...
                {hours.map((hour) => {
                  ...
                  const selected = recommendation?.utcHour === hour.utcHour;
                  return (
                    <div
                      className={`hour-cell ${working ? "working" : ""} ${selected ? "selected" : ""}`}
                      key={hour.utcHour}
                      title={`${person.name}: ${formatInZone(instant, person.timeZone)}`}
                    >
                      <span>{String(localHour).padStart(2, "0")}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <p className="timeline-note">Highlighted cells represent each person’s default working hours, 09:00–18:00.</p>
        </section>
        ...
        <section className="section launch-section">
          <div>
            ...
            <p>These MVP shortcuts open the provider. Automatic meeting creation comes later.</p>
          </div>
          <div className="launch-grid">
            ...
            <a href="viber://chat" target="_blank" rel="noreferrer"><Phone /> Viber <ExternalLink size={15} /></a>
          </div>
          {selectedInstant && (
            <p className="selected-summary">
              Selected instant: <strong>{selectedInstant.toUTCString()}</strong>
            </p>
          )}
        </section>
      </main>
