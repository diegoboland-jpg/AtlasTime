...
AtlasTime v1.0 completed the local-first cross-time-zone planner. v1.1–v1.10 added contacts, calendar availability, controlled sharing, and reproducible public staging. v1.11–v1.15 refined the phone workspace, Kikroo identity, Android preparation, privacy surface, and protected beta. v1.16 added explicit corporate time-zone abbreviation choices and approved the privacy-safe widget snapshot. v1.17 adds the first native Android home-screen widget and its verified local bridge.
...
Ownership and filing choices are recorded in the [legal decision record](docs/LEGAL_DECISION_RECORD.md) and [international trademark strategy](docs/INTERNATIONAL_TRADEMARK_STRATEGY.md). Protected-beta recruitment uses a completion-based [tester recruitment plan](docs/TESTER_RECRUITMENT_PLAN.md). Android distribution follows the [internal-test guide](docs/ANDROID_INTERNAL_TEST_GUIDE.md), while the first widget follows the reviewed [v1.17 architecture](docs/V1_17_ANDROID_WIDGET_ARCHITECTURE.md).
